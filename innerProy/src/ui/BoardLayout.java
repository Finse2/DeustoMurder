...
public final class BoardLayout extends JPanel {
    ...
    private void createBoard(List<Room> rooms) {
        ...
        for (Room room : rooms) {
            for (Room.Cell cell : room.getOccupiedCells()) {
                ...
                if (previousRoom != null) {
                    throw new IllegalArgumentException("Rooms '" + previousRoom.getName() + "' and '" + room.getName() + "' overlap at grid cell (" + cell.column() + ", " + cell.row() + ")");
                }
        ...
        for (int row = 0; row < GRID_ROWS; row++) {
            for (int column = 0; column < GRID_COLUMNS; column++) {
                ...
                add(tileLook);
                corridorTiles.add(new PlacedTile(tileLook, column, row));
            }
        }
    ...
    private void validateCell(Room room, Room.Cell cell) {
        if (cell.column() < 0 || cell.column() >= GRID_COLUMNS || cell.row() < 0 || cell.row() >= GRID_ROWS) {
            throw new IllegalArgumentException("Room '" + room.getName() + "' uses grid cell (" + cell.column() + ", " + cell.row() + ") outside the " + GRID_COLUMNS + "x" + GRID_ROWS + " board");
        }
    }

    @Override
    public void doLayout() {    //over-written to avoid problems during screen shrinking / expansions when Java Swing has to recalculate the size / position of each component
        super.doLayout();
        ...
        int tileSize = Math.min((availableWidth - margin * 2) / GRID_COLUMNS, (availableHeight - margin * 2) / GRID_ROWS);
        ...
        for (PlacedTile tile : corridorTiles) {
            tile.look().setBounds(startX + tile.column() * tileSize, startY + tile.row() * tileSize, tileSize, tileSize);
            tile.look().setGridLineThickness(
                    Math.max(1, tileSize / 24)
            );
        }
